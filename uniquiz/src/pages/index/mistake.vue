<template>
    <view>
        <view v-if="mistakes && mistakes.length > 0">
        ...
        <uni-pagination v-if="mistakes && mistakes.length > 0" :total="mistakes.length" :current="now_page"
            :pageSize="one_page_num" :show-icon="true" @change="page_change" />
    </view>
</template>

<script lang="ts" setup>
import { useMisStore } from '@/stores/mis'
...
const { req_mis } = store
const { mistakes } = storeToRefs(store)

// 当前页要显示的记录
const show_records = computed(() => {
    if (!mistakes.value || mistakes.value.length === 0) return []
